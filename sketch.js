...
function classify(l) {
  ...
  // ── 伸出食指（One Finger）：僅食指伸直，中指/無名指/小指均彎曲 ────────
  // 用於選單「繼續遊玩」
  if (ext[0] && !ext[1] && !ext[2] && !ext[3]) return 'one_finger';
...
function dMenu() {
  ...
  g.fillText('💡 ☝️ 伸出食指 2秒 → 🎮 繼續  ·  ✌️ 比耳 2秒 → 🏠 結束', W / 2, H / 2 + 106);
  g.restore();

  // 手勢選擇進度条
  if (st === 'menu' && (stable === 'one_finger' || stable === 'scissors')) {
    const pct = menuHoldT ? Math.min(1, (Date.now() - menuHoldT) / MENU_HOLD) : 0;
    const isContinue = stable === 'one_finger';
    ...
    // 顯示剩餘秒數
    const secLeft = Math.ceil(MENU_HOLD * (1 - pct) / 1000);
...
function update() {
  ...
  // ✌️ 比耳 保持 2 秒 → 結束遊戲
  if (st === 'menu') {
    if (stable === 'one_finger' || stable === 'scissors') {
      if (!menuHoldT) menuHoldT = now;
      if (now - menuHoldT >= MENU_HOLD) {
        if (stable === 'one_finger') startGame(); // 食指 → 繼續
        else enter('ended');                      // 剪刀/比耳 → 結束
        menuHoldT = null;
      }
    } else {
      menuHoldT = null; // 手勢消失或改變，重置計時
    }
  }
